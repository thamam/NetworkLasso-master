...
def solveX(data):
    ...
    for i in range(int(neighs.size/(2*inputs+1))):
        ...
        if(weight != 0):
            u = neighs[i*(2*inputs+1)+1:i*(2*inputs+1)+(inputs+1)].reshape(-1, 1)
            z = neighs[i*(2*inputs+1)+(inputs+1):(i+1)*(2*inputs+1)].reshape(-1, 1)
...
def main():
    ...
    file = open("Data/Sacramentorealestatetransactions_Normalized.csv", "r")
    ...
    while(lamb <= thresh or lamb == 0):
        ...
        for i in testList:
            ...
            for j in range(numNewNeighs):
                ...
                g = g + weight*cp.norm(xpred - x[:, it.GetKey()].reshape(-1, 1))
                it.Next()
